...
function tryParseHomerAppLegacySearch(
    params: URLSearchParams,
): SearchDeepLinkSpec | null {
    ...
    try {
        ...
        if (search && typeof search === "object") {
            for (const profile of Object.values(search)) {
                ...
                spec.node = firstNonEmpty(spec.node, p.node, p.node_id);
            }
        }
...
export function hasDeepLinkFilters(
    spec: SearchDeepLinkSpec | null | undefined,
): boolean {
...
export function parseSearchDeepLink(
    params?: URLSearchParams,
): SearchDeepLinkSpec | null {
    ...
    spec.node = firstNonEmpty(p.get("node"), p.get("node_id"));
...
export function buildSearchPayload(
    ...
    if (spec.node) filter.node = spec.node;
    return {
        filter,
        param: { limit: spec.limit ?? limit },
        timestamp:
            timestamp.from != null && timestamp.to != null ? timestamp : {},
    };
}
...
export function buildSearchDeepLinkURL(
    ...
    if (fields.node?.trim()) q.set("node", fields.node.trim());
    if (fields.proto_type != null && String(fields.proto_type) !== "") {
        q.set("proto_type", String(fields.proto_type));
    }
    if (fields.event_type?.trim())
        q.set("event_type", fields.event_type.trim());
    if (fields.limit != null && fields.limit > 0)
        q.set("limit", String(fields.limit));
...
export function deepLinkSpecToFormFields(
    spec: SearchDeepLinkSpec,
